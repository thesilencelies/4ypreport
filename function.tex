...
%this block doesn't seem to add much - I'll leave it here in case it is useful
%\begin{figure}
%\begin{algorithmic}
...
%        \EndFor
%    \EndFor
%    %needs typesetting as an algorithm
%   \State find min by checking all roots
%    
%  params used to make function of x thus:
%  function (coords, params)
%    local dim = params:size(1) 
%    local npar = params:size(2)
%    assert(coords:size(1) >= dim)
%    local out = 0
%    for i = 1, dim do
%        local res = 1/(npar + 1)   --(higest order term always has param of 1/it's order)
%        local x = coords[i]
%        for j = 1, npar do  
%        -- horner's rule
%	        res = res * x + (params[i][j] /(npar +1 -j))
%	    end
%	    out =  out + (res*x) -- we have no constant term, unlike with normal horners
%    end
%
%    return out
%end
...
%also should be detailing how the architecture works a lot more

%there also should be a bunch more sections here detailing all the other stuff I changed to improve it's performance:
%normalisation
%everystep
%DetRFO
%L2Norm!!!
