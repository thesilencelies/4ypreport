...
\node (x) at (0,0) [circ] { ~~$x$};
\node (fx)[circ, below right = of x] {$f(x)$}
	edge [<-] (x);
\node (fxmin) [circ, below right = of fx] {\footnotesize{$\hat x_{min}$}}
	edge [<-] (fx);
\node (inp) [layer, node distance = 1.8cm, below = of fx] {input size}
	edge [<-] (fx);
\draw (inp.north-|x.south) [<-] to (x);
\draw (inp.north-|fxmin.south) [<-] to (fxmin);
...
\node (ht) [NL, below= of noise] {HardTanh}
	edge [<-] (noise);
\node (xmul) [rectangle, fill = black!20, below = of ht] {$x_{max}$}
	edge [<-] (ht);
